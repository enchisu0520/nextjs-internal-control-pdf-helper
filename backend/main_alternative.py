...
# Try to import FAISS, fall back to our simple implementation if not available
try:
    from langchain_community.vectorstores import FAISS
    USE_FAISS = True
except ImportError:
    print("FAISS not available, using simple vector store implementation")
    USE_FAISS = False
    from vector_store_alternative import SimpleVectorStore
...
def create_vector_store(request_id, documents):
    if USE_FAISS:
        ...
        # Use our simple vector store implementation
        vectorstore = SimpleVectorStore(folder_path=folder_path)
        ...
        file_name = f"{request_id}.pkl"
        vectorstore.save_local(index_name=request_id, folder_path=folder_path)
        return file_name
...
def load_index(file_name):
    if USE_FAISS:
        ...
        # Load our simple vector store
        vectorstore = SimpleVectorStore.load_local(
            index_name=file_name.replace(".bin", ""),
            folder_path=folder_path,
            embeddings=bedrock_embeddings
        )
        return vectorstore
...
async def query(request: QueryRequest):
    try:
        file_name = f"{request.request_id}.bin" if USE_FAISS else f"{request.request_id}.pkl"
...
if __name__ == "__main__":
    ...
    uvicorn.run("main_alternative:app", host="0.0.0.0", port=port, reload=True) 